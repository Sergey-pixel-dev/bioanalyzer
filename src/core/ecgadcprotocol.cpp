...
constexpr int EcgAdcProtocol::kSampleRateHz[4];
constexpr int EcgAdcProtocol::kGainValues[7];
constexpr uint8_t EcgAdcProtocol::kTestFrequencies[2];

EcgAdcProtocol::EcgAdcProtocol(TransportProtocol *transport)
{
    setTransport(transport);
}

void EcgAdcProtocol::setTransport(TransportProtocol *transport)
{
...
int32_t EcgAdcProtocol::decodeSample(const uint8_t *p)
{
...
namespace
{
    ...
    TransportProtocol::ResponseHandler makeResultAdapter(EcgAdcProtocol::AckHandler onAck)
    {
        if (!onAck)
            return {};
...
int EcgAdcProtocol::startStream(const std::vector<uint8_t> &channels, AckHandler onAck)
{
...
int EcgAdcProtocol::stopStream(AckHandler onAck)
{
    if (m_transport == nullptr)
        return -1;
...
int EcgAdcProtocol::setVref(uint16_t mV, AckHandler onAck)
{
    if (m_transport == nullptr)
        return -1;
    if (mV > 3300)
        return -1;
    uint8_t payload[3] = {
        static_cast<uint8_t>(Cmd::SetVref),
...
int EcgAdcProtocol::setSamplerate(uint8_t idx, AckHandler onAck)
{
    if (m_transport == nullptr)
        return -1;
    if (idx > 0x03)
...
int EcgAdcProtocol::setGain(uint8_t channel, uint8_t gainCode, AckHandler onAck)
{
...
int EcgAdcProtocol::setShortInput(bool enable, AckHandler onAck)
{
    if (!enable)
        return setNormalInput(std::move(onAck));
    if (!m_transport)
        return -1;
...
int EcgAdcProtocol::setNormalInput(AckHandler onAck)
{
    if (!m_transport)
        return -1;
...
int EcgAdcProtocol::setTestSignal(uint8_t amplitude, uint8_t frequency, AckHandler onAck)
{
...
bool EcgAdcProtocol::decodeDeviceInfo(const uint8_t *payload, int len, DeviceInfo &out)
{
...
int EcgAdcProtocol::getDeviceInfo(DeviceInfoHandler onInfo)
{
    if (m_transport == nullptr)
        return -1;
...
void EcgAdcProtocol::onPush(const uint8_t *payload, int len, uint8_t seq)
{
